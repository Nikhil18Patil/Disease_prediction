...
def result(request):
    ...
    predicted_disease = DecisionTree(symptom1,symptom2,symptom3,symptom4,symptom5)
    obj = d_dis.objects.filter(name=predicted_disease).first()
    
    return  render(request, 'result.html',{'result':predicted_disease, 'd_dis':obj})
...
def period_calculator(request):
    ...
    if request.method == 'POST':
        ...
        period_start_date = (date.today() + timedelta(days=cycle_length)).strftime("%B %d, %Y")

        # Calculate most probable ovulation days
        ovulation_day_1 = (date.today() + timedelta(days=(cycle_length - 14))).strftime("%B %d, %Y")
        ovulation_day_2 = (date.today() + timedelta(days=(cycle_length - 15))).strftime("%B %d, %Y")
